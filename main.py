...
async def on_message(message):
	...
	if message.content == "servers":
		...
		channel_info = '\n'.join([f"Channel name: {name} | Channel ID: {channel_id}" for name, channel_id in channel_list])
	...
	if message.content == "tipo":
		...
		if channel_list == []:
			...
			channel_info = '\n'.join([f"Channel name: {name} | Channel Type: {channel_type}" for name, channel_type in channel_list])
			await message.channel.send(f"Canais:\n{channel_info}")
